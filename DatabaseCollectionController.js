...
let DatabaseConnectionFactory = require('./DatabaseConnectionFactory')
let DatabaseUtils             = require('./DatabaseUtils')

/**
 * Provides routes to access a collection.
 */
class DatabaseCollectionController extends CollectionController {
	/**
	...
	constructor(databaseConnectionFactory, table, options) {
		...
		if (options.excludeColumns !== undefined) {
			...
			if (this._columns === undefined)
				this._columns = this._databaseConnectionFactory.query(DatabaseConnectionFactory.ROLE_APP, db =>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	async _onDelete(ctx, next) {
		...
		await this._databaseConnectionFactory.query(ctx, (function(db) {
	...
	async _onDeleteItem(ctx, next, id) {
		// execute database query
		// TODO: Babel does not currently support lexical this in arrow functions during async/await
		let result = await this._databaseConnectionFactory.query(ctx, (function(db) {
	...
	async _onGet(ctx, next) {
		// execute database query
		// TODO: Babel does not currently support lexical this in arrow functions during async/await
		let result = await this._databaseConnectionFactory.query(ctx, (async function(db) {
			// select columns
...
module.exports = DatabaseCollectionController
